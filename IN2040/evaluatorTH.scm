...
;;Lagt til prosedyre som installerer primitive prosedyrer (i vanlig repl)
...
;;implementering av alternativ let syntax
(define (and? exp)
  (tagged-list? exp 'and))
  ...
  (define (set-params-inits exp)
    (if (or (and? exp) (let? exp))
        (begin
          ...
          (set! initials (cons (mc-eval (cadddr exp) env) initials))
